...
import { useAddPostsMutation, useDeletePostMutation, useGetPostQuery } from '@/redux/api/postApi';
import { AddPost_filter } from '@/components/admin/AddPost_filter';
...
export default function AdminPostsTable() {
  ...
  const { filterText, searchText } = useSelector(state => state.post)
  const { data, refetch } = useGetPostQuery({ filterText, searchText });// for all post 

  console.log(data?.data,'adfaf')
...
const [post]=useDeletePostMutation()
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 md:p-3 p-0 lg:p-6">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <AddPost_filter handleAddPost={handleAddPost} />
              ...
              <table className="w-full">
                ...
                <tbody>
                  {data?.data?.map((post, index) => (
                    <tr
                      key={post.id}
                      className={`border-b border-gray-100 hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 transition-all duration-300 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/30'
                        }`}
                    >
                      <td className="p-6">
